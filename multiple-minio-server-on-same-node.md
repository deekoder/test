# Running multiple Minio Server on single instance/machine. [![Gitter](https://badges.gitter.im/Join%20Chat.svg)](https://gitter.im/minio/minio?utm_source=badge&utm_medium=badge&utm_campaign=pr-badge&utm_content=badge)

![minio_MULTISERVER](https://raw.githubusercontent.com/koolhead17/test/master/docs/screenshots/image.jpeg?raw=true)

In this document we will illustrate how to set up multiple Minio server instance on single machine. All these Minio servers are running on thier own   port, data directory & configuration directory.
...
* Ports are empty for Minio server's use
...
```
**Testing it all**
Using Unix/Linux terminal comamnd ``netstat`` we can see ``Minio Server`` is running on different ports on same machine.
